...
@property (nonatomic, weak) RNDatamiFetch *networker;
         ...
         forTask:(RCTNetworkTask *)task
{
  ...
  if ([responseType isEqualToString:@"text"]) {
    ...
    responseString = [RNDatamiFetch decodeTextData:data fromResponse:task.response withCarryData:nil];
    if (!responseString) {
  ...
  if (incrementalUpdates) {
    if ([responseType isEqualToString:@"text"]) {
      ...
      incrementalDataBlock = ^(NSData *data, int64_t progress, int64_t total) {
        ...
        NSString *responseString = [RNDatamiFetch decodeTextData:data
...
@implementation RCTBridge (RNDatamiFetch)

- (RNDatamiFetch *)networking
{
  return [self moduleForClass:[RNDatamiFetch class]];
}
